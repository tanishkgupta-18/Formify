...
function FormUi({ jsonForm, selectedTheme, selectedBackground, onFieldUpdate, editable = true }) {
  const [formState, setFormState] = useState(jsonForm || null)

  useEffect(() => {
    setFormState(jsonForm)
  }, [jsonForm])
  ...
  return (
    <div
      className="max-w-2xl mx-auto p-6 rounded-lg shadow-md w-full"
      data-theme={selectedTheme}
      style={{ background: selectedBackground || 'inherit' }}
    >
                      ...
                      <div className="join-item flex">
                        <FieldEdit
                          ...
                          onUpdate={(updatedField) =>
                            handleUpdate(updatedField, sectionIndex, subIndex, "subheading")
                          }
                          onDelete={() =>
                            handleDelete(sectionIndex, subIndex, "subheading")
                          }
                        />
                      </div>
                    )}
                    ...
                    {editable && (
                      <div className="join-item flex">
                        <FieldEdit
                          defaultValue={field}
                          onUpdate={(updatedField) =>
                            handleUpdate(updatedField, sectionIndex, fieldIndex, "field")
                          }
                          onDelete={() =>
                            handleDelete(sectionIndex, fieldIndex, "field")
                          }
                        />
                      </div>
                    )}
